...
Based on the above observations, the distribution of averages of 40 exponentials is centered at `r mean(row_means)` and the same is close to the theoretical center of the distribution, which is $\lambda^{-1}$ = `r 1/lambda`.    
...
Based on the results, the variance in the distribution of averages of 40 exponentials is close to the theoretical variance of the distribution. The variance of exponentials means is `r var(row_means)`  and theoretical variance of the distribution is $\sigma^2 / n = 1/(\lambda^2 n)$ = `r 1/(((0.2) * (0.2)) * 40)`.  
